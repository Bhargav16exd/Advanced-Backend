...
const getWatchHistory = asyncHandler(async(req,res)=>{
    ...
    const historyResult = await User.aggregate([
        {
            $match:{
        ...
        },
        {
            $lookup:{
                ...
                foreignField:"_id",
                as:"userHistory",
                pipeline:[      // in video model we have ref to user to with each video owner ie user need to fetched
                    {                              // subpiplines
                        $lookup:{
                            from:"users",
                            localField:"owner",
                    ...
                    { // user returned will be obj with array  
                       $addFields:{
                          owner:{
                            $first:"$owner"
                          }
    ...
    .status(200)
    .json(
        new ApiResponse(200 ,getWatchHistory.watchHistory,"User history fetched success")
    )
})
export {
    registerUser , 
